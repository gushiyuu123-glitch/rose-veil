...
const titleColor = {
 white: "text-[#d9d6f2]",   // ← NEW（白背景で最強のラベンダーグレー）
  veil:  "text-[#f5c8d6]",  // → ピンクの温度 +5%、わずかに深みを追加
  blue:  "text-[#c8e2ff]",  // → 青みを少し強めて透ける高級感を保つ
};
...
const rightGrad = {
 ...
 white: `linear-gradient(to bottom,
    rgba(0,0,0,0.32),
    rgba(0,0,0,0.22),
    rgba(0,0,0,0.10)
  )`,
        ...
        <div className="h-full flex flex-col">
          ...
          <div className="px-7 pt-7 pb-5 border-b border-white/18">
            <div className="flex items-center justify-between">
              <Reveal delayMs={40}>
                <div className={[UI.t85, UI.trackHi, "text-[0.92rem]"].join(" ")}>
                  CART
                </div>
              </Reveal>
...
>
  ...
  />
                      ...
                      <div className="flex gap-4 items-start">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="mt-3 flex items-center justify-between">
                            <div className="text-white/80 text-[0.9rem] tracking-[0.06em]">
                              {yen(it.price)}
                            </div>
                            <button
                          ...
                          <div className="mt-4 flex items-center gap-3">
                            <button
                              ...
                              className={[
                                ...
                                "hover:border-white/28 transition",
                              ].join(" ")}
                            ...
                            <div className="text-white/80 tracking-[0.12em] text-[0.9rem] w-8 text-center">
                              {it.qty}
                            </div>
          ...
          <div className="px-7 py-6 border-t border-white/18">
            <Reveal delayMs={120}>
              <div className="flex items-center justify-between">
                <div className={[UI.t55, UI.trackHi, "text-[0.82rem]"].join(" ")}>
                  TOTAL
                </div>
                <div className="text-white/92 text-[1.06rem] tracking-[0.10em] font-light">
                  {yen(cartTotal)}
                </div>
              </div>
            </Reveal>

            {/* Dior/CHANEL checkout: 黒×白×0.18線 + 膜 */}
            <Reveal delayMs={200}>
              <button
                ...
                className={[
                  ...
                  cart.length === 0
                    ...
                    : "text-white/90 bg-white/7 hover:bg-white/9 hover:border-white/28",
                ].join(" ")}
              >
                <span className="relative inline-block">
                  CHECKOUT
                  {/* ultra-thin underline grows (inside button, Diorっぽい) */}
                  <span
                    aria-hidden="true"
                    className={[
...
export default function ProductOverlayWide() {
  ...
  function addToCart(p) {
    setCart((prev) => {
      ...
      return [
        ...prev,
        { key: p.key, title: p.title, sub: p.sub, price: p.price, image: p.image, qty: 1 },
      ];
    });
    setIsCartOpen(true);
  }
  function changeQty(key, delta) {
    setCart(prev => {
      const next = prev
        .map(it =>
          it.key === key ? { ...it, qty: it.qty + delta } : it
        )
        .filter(it => it.qty > 0); // ← qty0 を自動削除
      return next;
    });
  }
...
{/* ===== Top-right Cart Trigger（Dior系：Bagのみ） ===== */}
<div className="pointer-events-auto fixed top-3.5 right-5 z-[100]">
  ...
  >
    ...
    {cartCount > 0 && (
      <span className="text-white/60 text-[0.75rem] tracking-[0.10em]">
        {cartCount}
      </span>
    )}
  ...
  <div
    key={p.key}
    id={`product-${p.key}`}     // ← ★ 追加済み ★
    className="relative w-full border-b border-white/10"
  >
          
          {/* ===== PHOTO AREA ===== */}
          <div className="relative w-full h-[72vh] lg:h-[112vh] overflow-hidden">
            {/* ---- 背景写真（3D感：微差の影 + ピンライト） ---- */}
<div className="cinema-hover w-full h-full">
  ...
  <div className="cinema-shadow"></div>
              ...
              {/* 3D edge / vignette（静かな立体感） */}
              <div
                aria-hidden="true"
                className="absolute inset-0 pointer-events-none"
                style={{
                  background:
                    "linear-gradient(to bottom, rgba(0,0,0,0.20), rgba(0,0,0,0.02) 32%, rgba(0,0,0,0.22))",
...
gap-[8vw]
            ...
            >
              ...
              <Reveal delayMs={80 + i * 80} className="relative text-left translate-x-[1.5vw]">
                <h3 className={`${UI.t95} ${titleColor[p.key]} text-[2.8rem] font-light tracking-[0.02em]`}>
                  {p.title}
                </h3>
                {/* ★ WHITE視認性UP：背面に薄膜レイヤー ★ */}
{p.key === "white" && (
  <div
    aria-hidden="true"
    className="
  ...
  {p.metaCopy}
              ...
              <div className="relative text-left pl-[17vw] translate-x-[2vw]">
                <Reveal delayMs={160 + i * 80}>
                  <p className="text-white/95 text-[1.08rem] leading-[1.95] whitespace-pre-line">
                    {p.detail}
                  </p>
                </Reveal>
                ...
                <Reveal delayMs={310 + i * 80}>
                  <p className="mt-7 text-[1.3rem] text-white/95 tracking-wide font-light">
                    {p.priceLabel}
                  </p>
                </Reveal>
                ...
                <Reveal delayMs={380 + i * 80} className="pointer-events-auto mt-6">
                  <UnderlineAction onClick={() => addToCart(p)}>
                    PURCHASE
                  </UnderlineAction>
                </Reveal>
              </div>
          ...
          {/* ===== SP ===== */}
          <div className="lg:hidden bg-white px-6 py-10 text-left">
            <Reveal delayMs={60}>
              <h3 className="text-[1.8rem] font-light text-black/90 tracking-wide">
                {p.title}
              </h3>
              <p className="text-black/60 text-[1rem] mt-2">{p.sub}</p>
            </Reveal>

            <Reveal delayMs={120}>
              <p className="mt-6 text-[0.95rem] text-black/70 leading-[1.85] whitespace-pre-line">
                {p.detail}
              </p>
            </Reveal>

            <Reveal delayMs={180}>
              <p className="mt-6 text-[0.85rem] text-black/50 leading-relaxed">
                <span className="text-black/40 tracking-[0.12em]">INGREDIENTS</span>
                <br />
                {p.ingredients}
              </p>
            </Reveal>
            ...
            <Reveal delayMs={230}>
              <p className="mt-6 text-[1.15rem] text-black/80 tracking-wide font-light">
                {p.priceLabel}
              </p>
            </Reveal>
...
<button
  onClick={() => addToCart(p)}
  className="
    group relative mt-6 w-full py-3
